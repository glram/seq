...
std::shared_ptr<Attribute> AttributeHolder::getAttribute(std::string key) const {
    auto found = kvStore.find(key);
    return (found != kvStore.end())? found->second : std::shared_ptr<Attribute>{nullptr};
}

void AttributeHolder::setAttribute(std::string key, std::shared_ptr<Attribute> value) {
    kvStore[key] = value;
}
...
std::string AttributeHolder::textRepresentation() const {
    ...
    while (it != kvStore.end()) {
        it++;
        stream << it->first << "=" << it->second->textRepresentation() << ((it != kvStore.end())? ", " : "");
    }
    stream << "]";
    return stream.str();
}