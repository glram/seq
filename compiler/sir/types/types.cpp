...
std::shared_ptr<Type> kBytePointerType = std::make_shared<PointerType>(kByteType);
...
std::shared_ptr<Type> kStringPointerType = std::make_shared<PointerType>(kStringType);
std::shared_ptr<Type> kStringArrayType = std::make_shared<ArrayType>(
    std::static_pointer_cast<PointerType>(kStringPointerType));
...
void Type::resetId() {
  ...
  kBytePointerType = std::make_shared<PointerType>(kByteType);
  ...
  kStringPointerType = std::make_shared<PointerType>(kStringType);
  kStringArrayType = std::make_shared<ArrayType>(
      std::static_pointer_cast<PointerType>(kStringPointerType));
...
std::shared_ptr<Type> RecordType::getMemberType(const std::string &n) {
...
void RefType::accept(common::SIRVisitor &v) { v.visit(getShared<RefType>()); }

std::string RefType::textRepresentation() const {
  return fmt::format("{}: ref({})", referenceString(), contents->textRepresentation());
}

void FuncType::accept(common::SIRVisitor &v) { v.visit(getShared<FuncType>()); }
...
void PartialFuncType::accept(common::SIRVisitor &v) {
  v.visit(getShared<PartialFuncType>());
}
...
void PointerType::accept(common::SIRVisitor &v) { v.visit(getShared<PointerType>()); }

OptionalType::OptionalType(std::shared_ptr<PointerType> pointerBase)
    : RecordType(
          fmt::format(FMT_STRING("Optional[{}]"), pointerBase->getBase()->getName()),
          {kBoolType, pointerBase}, {"has", "val"}),
      base(pointerBase->getBase()) {}

void OptionalType::accept(common::SIRVisitor &v) { v.visit(getShared<OptionalType>()); }

ArrayType::ArrayType(std::shared_ptr<PointerType> pointerBase)
    : RecordType(
          fmt::format(FMT_STRING("Array[{}]"), pointerBase->getBase()->getName()),
          {kIntType, pointerBase}, {"len", "ptr"}),
      base(pointerBase->getBase()) {}

void ArrayType::accept(common::SIRVisitor &v) { v.visit(getShared<ArrayType>()); }

GeneratorType::GeneratorType(std::shared_ptr<Type> base)
    : Type(fmt::format(FMT_STRING("Generator[{}]"), base->getName())),
      base(std::move(base)) {}

void GeneratorType::accept(common::SIRVisitor &v) {
  v.visit(getShared<GeneratorType>());
}
...
void IntNType::accept(common::SIRVisitor &v) { v.visit(getShared<IntNType>()); }
