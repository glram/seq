...
void LowerFlowsVisitor::visit(VarValue *v) {}
void LowerFlowsVisitor::visit(PointerValue *v) {}
...
void LowerFlowsVisitor::visit(SeriesFlow *i) {
  for (auto &v : *i) {
    if (auto res = process(v)) {
      if (!isA<ValueProxy>(res)) {
        ctx->getInsertPoint()->push_back(move(res));
        if (isA<ControlFlowInstr>(res))
          setTerminator(move(res));
      }
    } else if (!isA<Flow>(v)) {
      ctx->getInsertPoint()->push_back(std::move(v));
    }
  }
}
...
void LowerFlowsVisitor::visit(WhileFlow *i) {
  ...
  process(i->getBody());
  setTerminator(ctx->module->Nxs<BranchInstr>(
      i->getSrcInfo(), makeProxy(condFlow, i->getSrcInfo())));
...
void LowerFlowsVisitor::visit(ForFlow *i) {
  ...
  process(i->getBody());
  setTerminator(ctx->module->Nxs<BranchInstr>(
      i->getSrcInfo(), makeProxy(updateFlow, i->getSrcInfo())));
...
void LowerFlowsVisitor::visit(TryCatchFlow *f) {
  ...
  for (auto &c : *f) {
    ...
    newTc->emplace_back(move(newHandler), c.type, c.catchVar);
  }

  ctx->getFrame()->push_back(move(newTc));
}

void LowerFlowsVisitor::defaultVisit(Constant *c) {}
...
void LowerFlowsVisitor::visit(StackAllocInstr *i) {
  if (auto count = process(i->getCount()))
    i->setCount(move(count));
}

void LowerFlowsVisitor::visit(YieldInInstr *i) {}
...
void LowerFlowsVisitor::visit(TernaryInstr *i) {
  ...
  auto namePrefix = i->getName().empty() ? fmt::format(FMT_STRING("ternary.{}"), i->getId())
                                         : i->referenceString();
...
void LowerFlowsVisitor::visit(BreakInstr *i) {
  auto *targetProxy = cast<ValueProxy>(i->getTarget());
  assert(targetProxy);
  auto *target = cast<Flow>(targetProxy->getValue());

  result =ctx->module->Nxs<BranchInstr>(i->getSrcInfo(),
                                         makeProxy(ctx->loopMappings[target->getId()].breakDst));
}
...
void LowerFlowsVisitor::visit(ContinueInstr *i) {
  ...
  result = ctx->module->Nxs<BranchInstr>(i->getSrcInfo(),
                                         makeProxy(ctx->loopMappings[target->getId()].continueDst));
}
